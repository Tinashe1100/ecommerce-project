<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="tw-mt-6 tw-space-y-6">
        ...
        <div>
            <x-input-label for="name" :value="__('Name')" class="w-text-gray-900 dark:tw-text-gray-100" />
            <x-text-input id="name" name="name" type="text" class="tw-mt-1 tw-block tw-w-full"
                :value="old('name', $user->name)" required autofocus autocomplete="name" />
            <x-input-error class="tw-mt-2 tw-text-gray-900 dark:tw-text-gray-100" :messages="$errors->get('name')" />
        </div>

        <div>
            <x-input-label for="email" :value="__('Email')" class="w-text-gray-900 dark:tw-text-gray-100" />
            <x-text-input id="email" name="email" type="email" class="tw-mt-1 tw-block tw-w-full"
                :value="old('email', $user->email)" required autocomplete="username" />
            <x-input-error class="tw-mt-2 tw-text-gray-900 dark:tw-text-gray-100" :messages="$errors->get('email')" />
        ...
        <div>
            <x-input-label for="phone" :value="__('Phone number')" class="w-text-gray-900 dark:tw-text-gray-100" />
            <x-text-input id="phone" name="phone_number" type="text" class="tw-mt-1 tw-block tw-w-full"
                :value="old('name', $user->phone_number)" required autofocus autocomplete="name" />
            <x-input-error class="tw-mt-2 tw-text-gray-900 dark:tw-text-gray-100" :messages="$errors->get('phone_number')" />
        </div>
